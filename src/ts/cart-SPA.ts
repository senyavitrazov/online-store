  ...
  if (window.location.pathname.split('/')[1] === 'cart') {
    ...
    if (totlalproducts)
      totlalproducts.innerHTML =
        total_in_cart.length > 0 ? `${total_in_cart.reduce((acc, el) => acc + el)}` : '0';
...
export const drawCart = (products: number[]): void => {
  ...
  if(pageNumbertext){
  drawproducts(array.slice(0+(parseInt(pageNumbertext.innerHTML)-1)*parseInt(pagelimitInput.value),array.length-(array.length-(parseInt(pageNumbertext.innerHTML)-1)*parseInt(pagelimitInput.value))+parseInt(pagelimitInput.value)),parseInt(pageNumbertext.innerHTML),parseInt(pagelimitInput.value));
    ...
    if (target.matches('.page__number-button-left')) {
      if (pageNumbertext) {
        ...
        drawproducts(
          ...
          ,parseInt(pageNumbertext.innerHTML),parseInt(pagelimitInput.value));
      }
    }
